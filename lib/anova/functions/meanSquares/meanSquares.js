...
function meanSquares(squaresSums, freedomDegrees) {
    ...
    var record = {
        ...
        calcs: [
            "$QME=frac{".concat(squaresSums.between, "}{").concat(freedomDegrees.between, "}=").concat(between, "$"),
            "$QMD=frac{".concat(squaresSums.within, "}{").concat(freedomDegrees.within, "}=").concat(within, "$"),
        ]
    };
    return {
        between: between,
